...
public class NagServiceHelper {
  ...
  private const string _stateFileNameTemplate =
      "Template10.Services.Nag.{0}.json";

  public NagServiceHelper(IDialogService dialogService,
                          IFileService fileService) {
    _dialogService =
        dialogService ?? throw new ArgumentNullException("dialogService");
    _fileService =
        fileService ?? throw new ArgumentNullException("fileService");
  }
  ...
  public async Task<bool> Exists(string nagId, NagStorageStrategies location) {
    ...
    return await _fileService.FileExistsAsync(
        string.Format(_stateFileNameTemplate, nagId));
  }

  public async Task<NagResponseInfo> Load(string nagId,
                                          NagStorageStrategies location) {
    if (string.IsNullOrEmpty(nagId)) {
      throw new ArgumentException("nagId cannot be null or empty", "nagId");
    }
    ...
    try {
      if (await Exists(nagId, location)) {
        return await _fileService.ReadFileAsync<NagResponseInfo>(
            GetFileName(nagId));
      }
  ...
  public async Task PersistAsync(NagResponseInfo state, string nagId,
                                 NagStorageStrategies location) {
    // TODO: use location
    ...
    await _fileService.WriteFileAsync<NagResponseInfo>(GetFileName(nagId),
                                                       state);
  }
  ...
  private async Task<NagResponse> ShowNag(NagEx nag) {
    ...
    if (nag.AllowDefer) {
      result = await _dialogService.PromptAsync(
          nag.Message, MessageBoxType.YesNoCancel, NagSettings.CustomResolver);
    } else {
      result = await _dialogService.PromptAsync(
          nag.Message, MessageBoxType.YesNo, NagSettings.CustomResolver);
    }
    switch (result) {
    case MessageBoxResult.Cancel:
      return NagResponse.Defer;
    case MessageBoxResult.No:
      return NagResponse.Decline;
    case MessageBoxResult.Yes:
      return NagResponse.Accept;
    default:
      return NagResponse.NoResponse;
    }
  }
